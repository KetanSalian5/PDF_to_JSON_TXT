#This is the update code we can able to change the format from pdf to json and json txt also(normal text) as per out requirements
...
def convert_pdf(pdf_path):
    ...
    for page_num in range(pdf_document.page_count):
        ...
        text_content += f"Page Number: {page_num + 1}\n{page_text}\n\n"

    pdf_document.close()
    return text_content.strip()  # Strip leading and trailing whitespaces

@app.route('/convert-pdf-txt-file', methods=['POST'])
def convert():
    if 'pdf' not in request.files:
    ...
    if file:
        pdf_path = 'temp.pdf'  # Save the uploaded PDF temporarily
        file.save(pdf_path)

        text_content = convert_pdf(pdf_path)

        if text_content:
            return text_content
        else:
            return jsonify({'error': 'Failed to convert PDF to text'})
